...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    elif "collecttime" in cols:
        rename_map[cols["collecttime"]] = "collect_time"
...
def build_dataloaders(
    ...
    # stride：滑窗步长（单位：单个采样点，15min粒度时 stride=12 表示3小时）
...
def do_training(
    *,
    ...
    if source == "csv":
        ...
        df = parse_time(df)
        series_df = resample_group(df, freq=freq)
    ...
    best_val = float("inf")
    no_improve = 0
    best_path = os.path.join(save, "informer_best.pt")
    ...
    try:
        ...
        # 2) forecast plot (first sample from first test batch) with residual injection curve
        forecast_path = os.path.join(save, "informer_forecast.png")
        ...
        # residual injection for visualization (IQR clean + random sampling)
        residuals = y_true - y_pred
        q1 = np.quantile(residuals, 0.25)
        q3 = np.quantile(residuals, 0.75)
        iqr = q3 - q1
        mask = (residuals >= q1 - 1.5 * iqr) & (residuals <= q3 + 1.5 * iqr)
        clean_res = residuals[mask]
        if clean_res.size > 5:
            sampled = np.random.choice(clean_res, size=len(y_pred), replace=True)
            y_pred_res = y_pred + sampled
        else:
            y_pred_res = y_pred

        plt.figure(figsize=(10, 4))
        plt.plot(y_true, label="true")
        plt.plot(y_pred, label="pred_raw", alpha=0.7, linestyle="--")
        plt.plot(y_pred_res, label="pred_residual_injected", alpha=0.9)
        plt.xlabel("step")
        plt.ylabel("value")
        plt.title("Forecast")
        plt.grid(True, alpha=0.3)
